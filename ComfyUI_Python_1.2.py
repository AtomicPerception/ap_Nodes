...
class PythonSnippet:
    ...
    def node_update(self, text, seed, code_inject=None, in1=None, in2=None, in3=None):
        ...
        s2 = ""
        s3 = ""
        if (in1!=None):s1 = str(in1)
        if (in2!=None):s2 = str(in2)
        if (in3!=None):s3 = str(in3)
        code_string = 'import random; PYTHON_CODE_BOX_SEED=%s;random.seed(PYTHON_CODE_BOX_SEED);IN1="%s";IN2="%s";IN3="%s";PREVIOUS_OUTPUT="%s";\n%s' %(seed,s1,s2,s3,self.last_output,text)
        if code_inject!=None:
