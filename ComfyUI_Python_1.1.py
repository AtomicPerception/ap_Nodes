...
class PythonSnippet:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {"code_inject": (AlwaysEqualProxy("*"),) }
        }
   
    def node_update(self, text, seed, code_inject=None):
        if code_inject!=None:
            # Grab code from the input to prefix the user's text box.
            code = code_inject
            code += 'import random; PYTHON_CODE_BOX_SEED=%s;random.seed(PYTHON_CODE_BOX_SEED);PREVIOUS_OUTPUT="%s";%s' %(seed,self.last_output,text)
        else:
        ...
        print("\033[36m[%s] PCB--> %s\033[0m" % (t,convert_result))
        self.last_output = convert_result
        return (code, convert_result, rf, ri,)

    @classmethod
    def IS_CHANGED(self, text, seed, code_inject = None):
        node_update(self, text, seed, code_inject)
        return ("",)
